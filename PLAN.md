...
| ユニットテスト | **Vitest** | Vite ベースで `vite.config.ts` を共有でき、TanStack Start (Vite) と相性が良い。ESM/TypeScript ネイティブ。サーバ関数のロジック (CRUD・権限チェック) を実 DB に当てて検証する。 |
| E2E テスト | **Playwright** | TanStack Start 公式ドキュメントでも推奨されているブラウザ E2E。ブラウザ実装に依存せずクロスブラウザで安定。ログイン → TODO 操作 → ログアウトの主要シナリオを担保する。 |
...
- **Jest**: 採用例は多いが、Vite/ESM/TS まわりの追加設定が増える。Vitest なら Vite 設定を共有でき設定が薄く済む。
- **Cypress**: E2E の選択肢として有力だが、Playwright の方がインストールが軽く、複数ブラウザ・並列実行・トレースが標準。
...
├── vitest.config.ts           # Vitest 設定 (Vite 設定を継承)
├── playwright.config.ts       # Playwright 設定
├── .env                       # DATABASE_URL, BETTER_AUTH_SECRET など (ローカル用としてコミットする)
├── drizzle/                   # マイグレーション SQL (drizzle-kit generate の出力)
├── tests/
│   ├── e2e/                   # Playwright シナリオ (ブラウザ起動を伴うので分離)
│   │   ├── auth.spec.ts       # サインアップ・ログイン・ログアウト・リダイレクト
│   │   └── todo.spec.ts       # TODO 操作・他ユーザ分離
│   └── helpers/
│       ├── db.ts              # テスト用 DB クライアント (TRUNCATE ヘルパ) — ユニット・E2E 共用
│       └── factory.ts         # テスト用ユーザ・TODO 生成 — ユニット・E2E 共用
└── src/
    ├── router.tsx             # ルータ生成
    ...
    └── features/
        └── todo/
            ...
            ├── server.test.ts # CRUD の Vitest ユニットテスト (co-location)
            └── components/    # 一覧・フォーム
```

> ユニットテストは **対象ファイルと同じディレクトリに `*.test.ts` で共置 (co-location)** する。Vitest 公式・TanStack Start (Vite) コミュニティで推奨される配置。E2E は性質が違う (ブラウザ起動・dev サーバ起動を伴う) ため `tests/e2e/` に分離する。
...
# テスト用 DB (sample_test) — ユニットテスト・E2E テストで利用 (dev データを壊さないため別 DB を切る)
TEST_DATABASE_URL=postgresql://db_user:db_password@localhost:5432/sample_test
...
- **対象**: TODO の CRUD サーバ関数 (`listTodos` / `createTodo` / `toggleTodo` / `deleteTodo`)。**「自分の TODO だけ操作・閲覧できる」** という権限ルールを担保することを最優先する。
- **配置**: テスト対象と **同じディレクトリ** に `*.test.ts` で共置する (co-location)。例: `src/features/todo/server.ts` ↔ `src/features/todo/server.test.ts`。
- **方針**: モックは原則使わず、**実 DB (PostgreSQL) に当てる**。Drizzle のクエリビルダや FK 制約も含めて検証することで、本番に近い挙動を確認できる。
- **DB 分離**: dev と同じ DB (`sample`) を共有すると開発データが壊れるため、Dev Container の PostgreSQL に **別 DB (`sample_test`)** を作成する。`.devcontainer/db/docker-entrypoint-initdb.d/` に `02-create-test-db.sql` を追加して `CREATE DATABASE sample_test;` する。
- **独立性**: 各テストの前に `users` / `accounts` / `sessions` / `todos` を `TRUNCATE ... CASCADE` してクリーンアップ。`beforeEach` でヘルパ関数を呼ぶ。
- **テストデータ**: `tests/helpers/factory.ts` にユーザ作成・TODO 作成のファクトリ関数を集約する。Better Auth は CLI 経由でユーザ作成するのが正攻法だが、テストでは Drizzle で直接 `users` / `accounts` に行を入れて手早く準備する (パスワードハッシュは Better Auth と同じ scrypt を使う、もしくは認証経路は E2E でカバーする想定でユニットでは ID だけ用意する)。
- **マイグレーション**: テスト前に `sample_test` に対しても `drizzle-kit migrate` を流す。`pnpm test` 内のセットアップで一度実行する。
- **Vitest 設定**: `include: ['src/**/*.test.ts']` を指定してソースツリー配下を拾う。E2E (`tests/e2e/`) は明示的に除外する。

### 8.2 E2E テスト (Playwright)

- **対象シナリオ**:
  1. サインアップ → ログイン → TODO 追加 → 完了切替 → 削除 → ログアウト (ハッピーパス)
  2. 未ログインで `/` にアクセス → `/login` にリダイレクトされる
  3. ユーザ A・B でサインアップ → A の TODO が B の画面に出ない (権限分離)
- **DB**: ユニットテストと同じ `sample_test` を使う。E2E 開始時に全テーブルを TRUNCATE。
- **サーバ起動**: Playwright の `webServer` 設定で `pnpm dev` (TEST_DATABASE_URL を上書き) を起動。
- **ブラウザ**: `chromium` のみ (CI 高速化)。必要が出たら追加する。
...
- `test`: `pnpm test:unit && pnpm test:e2e` (CI 用)
- `db:migrate:test`: `DATABASE_URL=$TEST_DATABASE_URL drizzle-kit migrate`
...
2. **PostgreSQL 接続 (Drizzle) とテスト用 DB 作成**
   - `drizzle-orm`, `pg`, `drizzle-kit`, `dotenv`, `@types/pg` を追加。
   - `drizzle.config.ts`, `src/db/client.ts` を作成。
   - `.devcontainer/db/docker-entrypoint-initdb.d/02-create-test-db.sql` を追加して `CREATE DATABASE sample_test;` する。既存ボリュームを使っている場合は `psql` で手動作成 (`createdb sample_test` 等) でも可。
...
6. **ユニットテスト導入 (Vitest)**
   - `vitest`, `@vitest/coverage-v8` (任意) を devDependencies に追加。
   - `vitest.config.ts` を作成 (`include: ['src/**/*.test.ts']`、テスト用 `.env` 読み込み)。
   - `tests/helpers/db.ts` (テスト用 Drizzle クライアント + `truncateAll()`) と `tests/helpers/factory.ts` (ユーザ・TODO ファクトリ) を作成。
   - `package.json` の `test:unit` / `db:migrate:test` を整える。`sample_test` に対して一度 `db:migrate:test` を流す。
   - `src/features/todo/server.test.ts` を作成し、CRUD と権限分離 (他ユーザの TODO を取得・更新・削除できないこと) をテスト。

7. **E2E テスト導入 (Playwright)**
   - `pnpm create playwright@latest --quiet` (もしくは `pnpm add -D @playwright/test && pnpm exec playwright install`) で導入。
   - `playwright.config.ts` を作成。`webServer` で `pnpm dev` を起動 (`DATABASE_URL=$TEST_DATABASE_URL` を渡す)。`baseURL=http://localhost:3000`、`projects` は `chromium` のみ。
   - `tests/e2e/auth.spec.ts`: サインアップ → ログイン → ログアウト、未ログイン時のリダイレクト。
   - `tests/e2e/todo.spec.ts`: TODO 追加・完了・削除のハッピーパス、ユーザ A/B 分離。
   - `globalSetup` でテーブル TRUNCATE。

8. **動作確認**
   - 2 ユーザでサインアップ → それぞれ TODO を作り、相手の TODO が見えないことを確認 (手動 + E2E)。
   - ログアウト → `/` にアクセスで `/login` にリダイレクトされることを確認。
   - `pgadmin4` (`http://localhost:8080`) で `users` / `accounts` / `sessions` / `todos` テーブルが期待通りに作られていることを確認。
   - `pnpm exec biome check .` がエラーなく通ることを確認。
   - `pnpm test:unit` / `pnpm test:e2e` がグリーンであることを確認。

9. **仕上げ**
   - `package.json` に `dev` / `build` / `lint` / `format` / `db:generate` / `db:migrate` / `db:migrate:test` / `db:studio` / `auth:generate` / `test:unit` / `test:e2e` / `test` のスクリプトを整理。
   - `.gitignore` に `drizzle/.snapshots/`, `routeTree.gen.ts`, `playwright-report/`, `test-results/` などを追加 (`.env` は含めない: §7 の方針通りローカル用としてコミットする)。
   - **README.md を本プロジェクトの最終ドキュメントとして書き起こす。** 構成案:
     ...
     7. テスト (`pnpm test:unit` / `pnpm test:e2e`、テスト DB の準備手順)
     8. DB マイグレーションの流れ (スキーマ変更 → `db:generate` → `db:migrate`)
     9. shadcn/ui コンポーネントの追加方法 (`pnpm dlx shadcn@latest add ...`)
     10. プロジェクト構成 (PLAN §4 のディレクトリ図を最終形に合わせて転記)
...
- **テスト DB のマイグレーション忘れ**: スキーマ変更後に `pnpm db:migrate` だけ流して `pnpm db:migrate:test` を忘れるとテストが落ちる。README とスクリプトで明示する。
- **E2E と dev サーバのポート競合**: Playwright の `webServer` が立てる dev サーバが、既に起動中の `pnpm dev` (3000 番) と衝突する可能性あり。E2E 実行時は dev サーバを止める方針を README に書く。
