...
`src/features/` 配下は **機能ドメインごと** にまとめる (feature-based)。1 つの機能の追加・改修・削除がフォルダ単位で完結するようにし、関連ファイルを物理的に近くに置く。

### ファイル命名規約 (実行環境の境界)

フルスタック構成では、サーバ専用コード (DB 接続・秘密鍵) がクライアントバンドルに混ざると情報漏洩につながる。これを防ぐため、**「間違えると事故るもの」だけサフィックスで明示的に縛る**。全ファイルを命名で分類しようとはしない (コンポーネントは種類が多く、サフィックスで実行環境を表しきれないため対象外)。

| サフィックス | 実行環境 | 縛り |
|---|---|---|
| `.server.ts` | **サーバ専用** | **厳守。** クライアントから import 禁止。`pg` / `betterAuth` など秘密情報・サーバ依存を含む。TanStack Start / Vite がこの命名を見てクライアントバンドルから除外する (本番ビルド後 `.output/public/assets/` を grep しても出てこない)。誤ってクライアントから import するとビルドエラーになる。`.middleware.ts` も実体はサーバで動くため同じ扱い。 |
| `.functions.ts` | **境界 (サーバ↔クライアント)** | **規約として明記。** `createServerFn` で公開する関数で、**クライアントから呼べる唯一の入口**。中身はサーバで実行され、クライアントへは RPC として呼び出される。新しいサーバ処理をクライアントに見せるときは必ずここを通す。 |
| `.schemas.ts` | **共有 (サーバ・クライアント両方)** | 慣習。Zod スキーマなど両側で使う純粋なコード (秘密情報は含めない)。サーバの `inputValidator` とクライアントのフォーム検証で同じスキーマを使う。どこから import してもよい。 |

上記以外 (コンポーネントの `.tsx`、`auth-client.ts` / `auth-errors.ts` などのクライアント寄りヘルパ) は**実行環境のサフィックス規約の対象外**。これらは決めるのは「置き場所」だけ:

- feature 固有の UI / ヘルパ → その feature フォルダ (例: `features/auth/auth-field-row.tsx`)
- 複数 feature で使い回す UI → `components/` (例: `components/ui/`)

> 注意: `-client` のような命名は**人間向けのラベルであって、ビルド上の除外効果は持たない** (`.server.ts` のみが除外される)。クライアント側ファイルは SSR でサーバでも実行されるため、`window` 必須の処理などは別途ガードする。
